...
import catalogConfigJson from "../../../../shandapha.catalog.json";

export const DEFAULT_CATALOG_CONFIG_PATH = "shandapha.catalog.json";

const staticCatalogConfig = assertCatalogConfig(catalogConfigJson);
const staticSourceManifests = new Map(
  [
    assertCatalogSourceManifest(acmeInternalCatalog),
    assertCatalogSourceManifest(communityDemoCatalog),
  ].map((manifest) => [manifest.source.id, manifest]),
);
...
export function loadCatalogConfig(catalogConfigPath?: string) {
  if (
    catalogConfigPath &&
    catalogConfigPath !== DEFAULT_CATALOG_CONFIG_PATH &&
    catalogConfigPath !== "./shandapha.catalog.json"
  ) {
    throw new Error(
      `Client-side catalog resolution only supports the built-in config. Received "${catalogConfigPath}".`,
    );
  }

  return cloneJson(staticCatalogConfig ?? createDefaultCatalogConfig());
}
...
function readCatalogSourceManifest(
  ...
  const manifest = staticSourceManifests.get(source.id);

  if (!manifest) {
    throw new Error(
      `Catalog source "${source.id}" is unavailable in the browser-safe registry client.`,
    );
  }
  ...
  config.sources
    ...
    .forEach((source) => {
      ...
      const sourceManifest = overlaySourceManifest
        ...
        : source.kind === "file"
          ? readCatalogSourceManifest(source)
          : undefined;
